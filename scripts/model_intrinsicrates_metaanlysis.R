...
fitted_br1_gnls <- gnls(r~briere1(a,temp,Tmin,Tmax),
                        data = train_data,
                        start = list(a = 0.00017,
                                     Tmin =8,
                                     Tmax= 39),
                        weights = varPower())
ff=summary(fitted_br1_gnls)
powerVar <- ff$modelStruct$varStruct[1]
shapiro.test(ff$residuals)
hist(ff$residuals)
summary(fitted_br1_gnls)
plot(fitted_br1)
plot(fitted_br1_gnls)
fitted_br1$m$
resid(fitted_br1)
...
fitted_br1_1 <- gnls(growth_rate ~ briere1(a,temp = temperature,Tmin,Tmax),
                     data = IR_data_ID1,
                     start = list(a = 0.00008,
                                  Tmin =4.5,
                                  Tmax= 33.5),
                     weights = varPower(),
                     control= gnlsControl(nlsTol=0.05))
sum_br1_1 <- summary(fitted_br1_1)
powerVar_1 <- sum_br1_1$modelStruct$varStruct[1]
coefs_1 <- data.frame(sum_br1_1$tTable[,1:2],row.names = NULL)
...
fitted_br1_2 <- nls(growth_rate ~ briere1(a,temp = temperature,Tmin,Tmax),
                    data = IR_data_ID2,
                    start = list(a = 0.00003,
                                 Tmin =10,
                                 Tmax= 40))
                    weights = varPower(),
                    control= gnlsControl(nlsTol=150))
plot(fitted_br1_2)
...
fitted_br1_3 <- gnls(growth_rate ~ briere1(a,temp = temperature,Tmin,Tmax),
                    data = IR_data_ID3,
                    start = list(a=0.00015,Tmin=6.5,Tmax=29.5),
                    weights = varPower(),
                    control = gnlsControl(minScale = 0.1))
...
fitted_br1_7_gnls <- gnls(growth_rate ~ briere1(a,temp = temperature,Tmin,Tmax),
                          data = IR_data_ID7,
                          start = list(a=0.00009,Tmin=5,Tmax=35),
                          weights = varPower())
sum_br1_7 <- summary(fitted_br1_7)
summary(fitted_br1_7_gnls)
