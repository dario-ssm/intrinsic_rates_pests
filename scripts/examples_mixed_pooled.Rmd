...
= **0.31** with a BCI of [0.21, 0.46]; the estimate (*summary effect)*
is **0.43**, with a 95% BCI [0.28, 0.57].
...
argumento `weights`. Pero, al ser un metaanálisis, debemos ponderar
cada estudio en función de su propia varianza muestral, de modo que
debemos también incorporar un `varFixed(~vi)`. Para combinar las dos,
utilizamos `varComb(varFixed(etc),varExp(etc))`. A continuación
mostramos los outputs de todas las combinaciones teóricamente posibles.
...
Funciona bien el modelo nlme. Según Wolfgang Viechtbauer, el creador
del paquete **`metafor`**, habría que añadir un
`(n)lmeControl(sigma = 1)` :
...
Por tanto, las *between-studies variances* para los distintos
parámetros apenas varían pero sí lo hace la *within-studies*
variance, pues al no fijarla en 1 queda como 1.213.
